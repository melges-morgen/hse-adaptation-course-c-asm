...
Показать, из каких основных частей состоит компьютер, где хранятся данные и команды, и почему будущему программисту на C и NASM нужно понимать память, адреса и регистры.

\subsection*{Обязательные темы}
\begin{itemize}
    \item CPU, RAM, SSD/HDD, GPU: роли основных компонентов компьютера.
    \item Бит, байт, машинное слово, адрес, адресное пространство.
    \item Регистр процессора как самая быстрая рабочая память CPU.
    \item Оперативная память как массив байтов с адресами.
    \item Отличие хранения программы на диске от исполнения программы в памяти.
    \item Стек и куча на уровне идеи: где появляются локальные переменные и динамические данные.
    \item Связь с C: переменная, указатель, массив, размер типа.
    \item Связь с NASM: регистры, инструкции, обращение к памяти.
\end{itemize}

\subsection*{Усложнённые темы}
\begin{itemize}
    \item Иерархия памяти: регистры, кэш, RAM, SSD/HDD.
    \item Почему доступ к разным уровням памяти имеет разную стоимость.
    \item Кэш-линии и локальность данных на уровне общей идеи.
    \item Порядок байт в памяти: little-endian и big-endian.
    \item Виртуальная память и страницы как подготовка к теме операционных систем.
\end{itemize}

\subsection*{Ключевые технологии и понятия}
\begin{itemize}
    \item x86-64 / AMD64.
    \item Register, memory, address, stack, heap.
    \item \texttt{sizeof}, адрес переменной, указатель.
    \item \texttt{hexdump}, \texttt{xxd}, \texttt{objdump} как инструменты для демонстрации представления данных.
\end{itemize}
